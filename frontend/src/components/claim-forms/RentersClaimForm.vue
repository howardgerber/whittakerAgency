<template>
  <div class="renters-claim-form">
    <h3>Renters Insurance Claim Details</h3>
    ...
    <div class="form-group">
      ...
      <div class="checkbox-group">
        <label class="checkbox-label"><input type="checkbox" v-model="formData.items_affected" value="electronics" /> Electronics</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.items_affected" value="furniture" /> Furniture</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.items_affected" value="clothing" /> Clothing</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.items_affected" value="jewelry" /> Jewelry</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.items_affected" value="appliances" /> Appliances</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.items_affected" value="other_personal_property" /> Other Personal Property</label>
      </div>
      <small v-if="touched.items_affected && !isItemsAffectedValid" class="below-min">Please select at least one item type</small>
    </div>
    ...
    <div class="form-group">
      <label>Landlord Notified? *</label>
      <div class="radio-group">
        <label class="radio-label"><input type="radio" v-model="formData.landlord_notified" value="yes" /> Yes</label>
        <label class="radio-label"><input type="radio" v-model="formData.landlord_notified" value="no" /> No</label>
      </div>
    </div>
...
interface RentersClaimFormData {
  incident_type: string
  items_affected: string[]
  police_report_filed: string
  police_report_number: string
  police_department: string
  landlord_notified: string
  damage_description: string
}

const emit = defineEmits<{
  (e: 'update', data: RentersClaimFormData): void
  (e: 'valid', isValid: boolean): void
}>()

const formData = reactive<RentersClaimFormData>({
  incident_type: '',
  items_affected: [],
  police_report_filed: '',
...
const isItemsAffectedValid = computed(() => formData.items_affected.length > 0)
...
const isFormValid = computed(() => {
  const baseValid = (
    ...
    isItemsAffectedValid.value &&
    formData.police_report_filed !== '' &&
    formData.landlord_notified !== '' &&
