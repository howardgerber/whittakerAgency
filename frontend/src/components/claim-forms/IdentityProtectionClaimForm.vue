<template>
  <div class="identity-protection-claim-form">
    <h3>Identity Protection Claim Details</h3>
    ...
    <div class="form-group">
      ...
      <select id="incident-type" v-model="formData.incident_type" required class="form-control">
        ...
        <option value="bank_account_fraud">Bank Account Fraud</option>
        <option value="identity_theft">Identity Theft</option>
        <option value="data_breach">Data Breach</option>
        <option value="phishing">Phishing</option>
        <option value="tax_fraud">Tax Fraud</option>
        <option value="medical_identity_theft">Medical Identity Theft</option>
        <option value="other">Other</option>
      </select>
    </div>

    <div class="form-group">
      <label for="date-discovered">Date Discovered *</label>
      <input
        type="date"
        id="date-discovered"
        v-model="formData.date_discovered"
        required
        :max="maxDate"
        class="form-control"
      />
    ...
    <div class="form-group">
      <label>Accounts Affected *</label>
      <div class="checkbox-group">
        <label class="checkbox-label"><input type="checkbox" v-model="formData.accounts_affected" value="bank_account" /> Bank Account</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.accounts_affected" value="credit_card" /> Credit Card</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.accounts_affected" value="investment_account" /> Investment Account</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.accounts_affected" value="social_security" /> Social Security</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.accounts_affected" value="medical_records" /> Medical Records</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.accounts_affected" value="tax_records" /> Tax Records</label>
        <label class="checkbox-label"><input type="checkbox" v-model="formData.accounts_affected" value="other" /> Other</label>
      </div>
      <small v-if="touched.accounts_affected && !isAccountsAffectedValid" class="below-min">Please select at least one account type</small>
    </div>
    ...
    <div class="form-group">
      <label>Financial Institutions Notified? *</label>
      <div class="radio-group">
        <label class="radio-label"><input type="radio" v-model="formData.financial_institutions_notified" value="yes" /> Yes</label>
        <label class="radio-label"><input type="radio" v-model="formData.financial_institutions_notified" value="no" /> No</label>
      </div>
    </div>

    <div class="form-group">
      <label>Credit Bureaus Notified? *</label>
      <div class="radio-group">
        <label class="radio-label"><input type="radio" v-model="formData.credit_bureaus_notified" value="yes" /> Yes</label>
        <label class="radio-label"><input type="radio" v-model="formData.credit_bureaus_notified" value="no" /> No</label>
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <textarea
        ...
        placeholder="Describe the identity protection incident"
...
interface IdentityProtectionClaimFormData {
  ...
  financial_institutions_notified: string
  credit_bureaus_notified: string
  incident_description: string
}

const emit = defineEmits<{
  (e: 'update', data: IdentityProtectionClaimFormData): void
  (e: 'valid', isValid: boolean): void
}>()

const formData = reactive<IdentityProtectionClaimFormData>({
  incident_type: '',
  date_discovered: '',
  accounts_affected: [],
  police_report_filed: '',
  ...
  police_department: '',
  financial_institutions_notified: '',
  credit_bureaus_notified: '',
  incident_description: ''
})

const touched = reactive({
  accounts_affected: false,
...
const isAccountsAffectedValid = computed(() => formData.accounts_affected.length > 0)
...
const isFormValid = computed(() => {
  const baseValid = (
    ...
    formData.date_discovered !== '' &&
    isAccountsAffectedValid.value &&
    formData.police_report_filed !== '' &&
    formData.financial_institutions_notified !== '' &&
    formData.credit_bureaus_notified !== '' &&
...
<style scoped>
.identity-protection-claim-form { width: 100%; }
